...
// import excel "E:\Desktop\Desktop\市场化与健康项目\Geo_Data\地理数据\海拔与坡度\海拔与坡度.xlsx", sheet("Sheet1") firstrow clear
// rename 海拔米 海拔平均值
// rename F 海拔最大值
// rename G 海拔最小值
// rename H 海拔标准差
...
// rename M 坡度标准值
// drop in 1
// destring 海拔平均值-坡度标准值, replace force
// gen mountainous=1 if 海拔平均值>500 & 海拔平均值<.
// replace mountainous=0 if 海拔平均值<=500
// save mountainous, replace


// cd "E:\Desktop\Desktop\市场化与健康项目\Geo_Data\地理数据\气象数据\TEM"
// local myfilelist : dir . files "*.txt"
// foreach file of local myfilelist {
//               import delimited "`f'", delimiter(space, collapse) encoding(Big5) clear
//               local file1= subinstr("`file'","SURF_CLI_CHN_MUL_DAY-TEM-12001-","",.)
//               local outfile = subinstr("`file1'",".txt","",.)
//               save "geo_`outfile'", replace
// }

// clear all
// cd E:\Desktop\Desktop\市场化与健康项目\Geo_Data\地理数据\气象数据
// use tem, clear
// // preserve
// gen date = mdy(v6, v7, v5)
// format date %td
// xtset v1 date
// bys v1: gen avg5_temp_mean=(F2.v8 +F1.v8 + v8 + L1.v8 + L2.v8) / 5
// gen tmp22=1 if avg5_temp_mean>220 & avg5_temp_mean<.
// replace tmp22=0 if avg5_temp_mean<=220

// restore
// rename v1 站点号
// drop v2-month
// duplicates drop 站点号, force

// bys station_id: egen avg_temp_mean=mean(temperature_avg)
// bys station_id: egen avg_temp_median=median(temperature_avg)
// bys station_id: egen max_temp_mean=mean(temperature_maxday)
// bys station_id: egen max_temp_median=median(temperature_maxday)
// bys station_id: egen min_temp_mean=mean(temperature_minday)
// bys station_id: egen min_temp_median=median(temperature_minday)
// bys station_id Year: egen pre_year=total(precipitation_day)
// bys station_id Month: egen pre_month=total(precipitation_day)
// bys station_id: egen pre_median=median(precipitation_day)
// label var avg_temp_mean "日平均温度的平均值"
// label var avg_temp_median "日平均温度的中位数"
// label var max_temp_mean "日最高温度的平均值"
// label var max_temp_median "日最高温度的中位数"
// label var min_temp_mean "日最低温度的平均值"
// label var min_temp_median "日最低温度的中位数"
// label var pre_mean "日累计降水量的平均值"
// label var pre_median "日累计降水量的中位数"
...
// bys station_id: egen min_temp_median=median(temperature_minday)
// bys station_id Year: egen pre_year=total(precipitation_day)
// bys station_id: egen pre_year_avg=mean(pre_year)
// bys station_id Year Month: egen pre_month=total(precipitation_day)
// bys station_id: egen pre_month_avg=median(pre_month)
// label var avg_temp_mean "日平均温度的平均值"
// label var avg_temp_median "日平均温度的中位数"
// label var max_temp_mean "日最高温度的平均值"
// label var max_temp_median "日最高温度的中位数"
